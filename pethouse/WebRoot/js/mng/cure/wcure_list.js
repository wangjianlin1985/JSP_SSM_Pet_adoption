var fmt = (function() {
	...
	fmt.fmtOpt = function(val, row, index) {
		return '<a href="javascript:void(0);" onclick="fmt.edit(' + row.id+ ',' + row.staffId+ ');">已医治</a> ';
	};
	fmt.edit = function(id,staffId) {
		if(confirm('是否确认已医治好该宠物')){
			$.ajax({
				url : 'm/cure/cure',// 跳转到 action  
				type : 'post',
				cache : false,
...
$(function() {
	...
	var pet_toolbar = [ {
		...
		iconCls : 'icon-reload',
		handler : function() {
			$("#pet_datagrid").datagrid("options").url = 'm/cure/findPage?statue=5';
			$('#pet_datagrid').datagrid('reload');
		}
	} ];
